...
async def docker_container_state(container_name):
    """Return Docker container state, or None if the container does not exist."""
    try:
        return await execute_docker(
            "docker inspect -f '{{.State.Status}}' " + shlex.quote(container_name),
            timeout=15
        )
    except Exception:
        return None
...
def _size_gb(value, default=30):
    """Convert values such as 30GB/30 to an integer GB value."""
    try:
        return int(float(str(value).upper().replace('GB', '').strip()))
    except (TypeError, ValueError):
        return default


async def ensure_vps_container(vps):
    """Ensure a saved VPS has a usable Docker container before SSH/SSHX access."""
    container_name = vps.get("container_name")
    if not container_name:
        raise Exception("VPS container name is missing from saved VPS data")

    state = await docker_container_state(container_name)
    if state is None:
        ram_gb = _size_gb(vps.get("ram", "1GB"), 1)
        disk_gb = _size_gb(vps.get("storage", "30GB"), 30)
        try:
            cpu = int(float(str(vps.get("cpu", 1)).split()[0]))
        except (TypeError, ValueError):
            cpu = 1
        password = vps.get("ssh_password") or generate_password()
        await create_docker_container(
            container_name, ram_gb * 1024, cpu, 0, password, disk_gb=disk_gb
        )
    ...
    if state != "running":
        await execute_docker("docker start " + shlex.quote(container_name), timeout=120)
        await asyncio.sleep(1)

    # Make sure SSH daemon is available after recovery/start.
    await docker_exec(container_name, "/usr/sbin/sshd || true", timeout=15)
    vps["status"] = "running"
    save_data()
...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30):
    ...
    # Reuse an existing container with the same name instead of failing with
    # "Conflict. The container name is already in use".
    existing_state = await docker_container_state(container_name)
...
async def get_sshx_session(container_name):
    ...
    for _ in range(20):
        ...
        if match:
            url = match.group(0)
            # Remove terminal ANSI/reset fragments such as [0m from sshx output.
...
if __name__ == "__main__":
    ...
    if not token:
        raise SystemExit("DISCORD_TOKEN is not set. Set your Discord bot token in the environment before starting the bot.")
    bot.run(token)